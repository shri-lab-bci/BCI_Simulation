...
void callback_marker3d_infos(const sc_interaction::ObjectInfos::ConstPtr& msg) {
...
int main(int argc, char** argv){
    ros::init(argc, argv, "marker_tf_lis");
    ros::NodeHandle node;

    ros::Publisher marker3d_info_tf_pub = node.advertise<sc_interaction::ObjectInfos>("/recognition/marker3d_infos_tf", 10);
    ros::Subscriber marker3d_info_sub = node.subscribe("/recognition/marker3d_infos", 10,  &callback_marker3d_infos);
    ...
    while(ros::ok())
    {
        ...
        marker3d_info_tf_pub.publish(g_aDataForPub);
        ros::spinOnce();
        r.sleep();
    }
}
